...
import {Stage, PartyMember, UserCharacter, StatEntry, CustomField, Condition} from "./Stage";
...
export function PartyTrackerUI({ stage }: { stage: Stage }): ReactElement {
    ...
    // Active tab state
    const [activeTab, setActiveTab] = useState<'user' | 'party' | 'quests' | 'world'>('user');
    ...
    // Add user stat
    const addUserStat = () => {
        const newStats = [...partyData.userCharacter.stats, { label: '', currentValue: '', maxValue: '' }];
        updateUserCharacter('stats', newStats);
    };

    // Update user stat
    const updateUserStat = (statIndex: number, field: keyof StatEntry, value: string) => {
        const newStats = partyData.userCharacter.stats.map((stat, idx) => 
    ...
    const removeUserStat = (statIndex: number) => {
        const newStats = partyData.userCharacter.stats.filter((_, idx) => idx !== statIndex);
        updateUserCharacter('stats', newStats);
    };
    ...
    const addInventoryItem = (item: string) => {
        if (item.trim()) {
            const newInventory = [...partyData.userCharacter.inventory, item.trim()];
    ...
    const removeInventoryItem = (index: number) => {
        const newInventory = partyData.userCharacter.inventory.filter((_, idx) => idx !== index);
        updateUserCharacter('inventory', newInventory);
    };

    // Generate injection preview for user character
    const getUserInjectionPreview = (): string => {
        const user = partyData.userCharacter;
        let info = `[USER CHARACTER`;
        if (user.name.trim()) {
            info += `: ${user.name}`;
        }
        if (user.class.trim()) {
                ...
                {activeTab === 'user' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        ...
                        <div style={{
                            ...
                            <input
                                type="text"
                                value={partyData.userCharacter.name}
                            ...
                            <input
                                type="text"
                                value={partyData.userCharacter.class}
                            ...
                            <textarea
                                value={partyData.userCharacter.description}
                        ...
                        <div style={{
                            ...
                            {partyData.userCharacter.stats.map((stat, idx) => (
                                <div
                                    key={idx}
                                    style={{
                                        display: 'flex',
                        ...
                        <div style={{
                            ...
                            {partyData.userCharacter.inventory.length > 0 ? (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                    {partyData.userCharacter.inventory.map((item, idx) => (
                                        <div
                                            key={idx}
                                            style={{
                                                display: 'flex',
                        ...
                        <div style={{
                            ...
                            <textarea
                                value={partyData.userCharacter.notes}
                        ...
                        <div style={{
                            ...
                            <button
                                onClick={() => updateUserCharacter('showDebug', !partyData.userCharacter.showDebug)}
                                style={{
                                    width: '100%',
                                    display: 'flex',
                                    alignItems: 'center',
                            ...
                            >
                                ...
                                <span>{partyData.userCharacter.showDebug ? '▼' : '▶'} Debug Info</span>
                            </button>

                            {partyData.userCharacter.showDebug && (
